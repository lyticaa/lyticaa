...
func (a *App) Start() {
	a.Logger.Info().Msgf("listening for messages on %v....", a.queue())
	...
	go func() {
		<-a.Signalling.quit
		a.Logger.Info().Msgf("stop listening on %v....", a.queue())
		a.Signalling.cancel()
		_ = conn.Close()
		_ = ch.Close()
	}()
...
func (a *App) declare(ch *amqp.Channel) (amqp.Queue, error) {
	return ch.QueueDeclare(a.queue(), true, true, false, false, nil)
}
